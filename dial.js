...
Dial.prototype.start = function(index) {
	...
	if(index != undefined) {
		//计算结束角度
		this.finishDegree = this.rounds + (this.reverse ? 1-(index%this.length +.5)/this.length : (index%this.length +.5)/this.length);
...
Dial.prototype.stop = function(index) {
	...
	if(index != undefined) {
		//计算结束角度
		this.finishDegree = this.counter + this.rounds + (this.reverse ? 1-(index%this.length +.5)/this.length : (index%this.length +.5)/this.length);
	} else {
		this.finishDegree = this.reverse ? 1-(this.index +.5)/this.length : (this.index +.5)/this.length;
	}
};
